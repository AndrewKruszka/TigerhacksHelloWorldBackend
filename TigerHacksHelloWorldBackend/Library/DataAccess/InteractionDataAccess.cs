...
{
	public class InteractionDataAccess : IInteractionDataAccess
	{
        private readonly ISettings settings;

        public InteractionDataAccess(ISettings settings)
		{
            this.settings = settings;
        }

        public IEnumerable<Interaction> GetInteractions(Int64 userId)
        {
            try
            {
        ...
        public Result UpsertInteraction(Int64 primaryUserId, Int64 interactedWithId, string geohash)
        {
            ...
            {
                using (var connection = NpgsqlDataSource.Create(settings.DB))
                using (var command = connection.CreateCommand("mw_create_interaction"))
                {
